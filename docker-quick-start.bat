...
echo ========================================
echo Flux Generator with ComfyUI - Docker Quick Start
echo ========================================
echo.
...
docker stop flux-generator-comfy 2>nul
docker rm -f flux-generator-comfy 2>nul
REM Also clean up old container name if it exists
docker stop flux-prompt-web 2>nul
docker rm -f flux-prompt-web 2>nul
REM Then try compose commands
docker compose --profile web stop flux-generator-comfy 2>nul
docker compose --profile web rm -f flux-generator-comfy 2>nul
...
docker compose --profile web up --build -d flux-generator-comfy

if errorlevel 1 (
  echo [ERROR] Failed to start the container. Check Docker logs for details.
  echo.
  echo Run: docker compose logs flux-generator-comfy
  pause
  exit /b 1
)
...
docker compose --profile web ps flux-generator-comfy | findstr "Up" >nul
if errorlevel 1 (
  echo [WARNING] Container may not have started properly. Check logs with:
  echo   docker compose logs flux-generator-comfy
  echo.
) else (
...
echo   View logs:    docker compose logs -f flux-generator-comfy
echo   Stop app:     docker compose --profile web stop flux-generator-comfy
echo   Restart app:  docker compose --profile web restart flux-generator-comfy
echo.
